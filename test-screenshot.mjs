...
await p.setViewport({ width: 794, height: 1123 }); // A4 at 96dpi
...
// Get the total height of the PDF content
const dims = await p.evaluate(() => {
  const modal = document.querySelector('#modal-pdf .modal');
  const pdfDoc = modal.querySelector('.pdf-doc');
  const page1 = modal.querySelector('.pdf-page');
  const page2 = modal.querySelectorAll('.pdf-page')[1];
  return {
    modalHeight: modal.scrollHeight,
    pdfDocHeight: pdfDoc ? pdfDoc.scrollHeight : null,
    page1Height: page1 ? page1.scrollHeight : null,
    page2Height: page2 ? page2.scrollHeight : null,
    page1OffsetTop: page1 ? page1.offsetTop : null,
    page2OffsetTop: page2 ? page2.offsetTop : null,
    windowWidth: window.innerWidth,
    pdfDocWidth: pdfDoc ? pdfDoc.offsetWidth : null,
    page1Width: page1 ? page1.offsetWidth : null,
  };
});
console.log('Dimensions:', JSON.stringify(dims, null, 2));

// Screenshot the full page
await p.screenshot({ path: 'test-screenshot.png', fullPage: true });
